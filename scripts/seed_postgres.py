    ...
    try:
        ...
        if cur.fetchone()[0] < 3:
            ...
            if not all(os.environ.get(f"SANDARB_{k.upper()}_SECRET") for k in secrets_map):
                # Write secrets to a file only (never to stdout/logs) to avoid exposure in Cloud Logging etc.
                repo_root = Path(__file__).resolve().parent.parent
                secrets_file = repo_root / ".env.seed.generated"
                lines = ["# Generated by seed_postgres.py. Add to .env and restart. Do not commit.\n"]
                for k, v in secrets_map.items():
                    lines.append(f"SANDARB_{k.upper()}_SECRET={v}\n")
                try:
                    secrets_file.write_text("".join(lines))
                    secrets_file.chmod(0o600)
                    print("\nService account secrets written to .env.seed.generated (mode 0600). Add to .env and restart. Do not commit.", file=sys.stderr)
                except Exception as e:
                    print(f"\nWarning: could not write secrets to .env.seed.generated: {e}. Set SANDARB_UI_SECRET, SANDARB_API_SECRET, SANDARB_A2A_SECRET in .env manually.", file=sys.stderr)
