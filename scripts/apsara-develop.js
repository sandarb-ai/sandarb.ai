#!/usr/bin/env node
/**
 * 🪷 Sandarb.ai Apsara Development Orchestrator
 * 
 * Assigns enterprise-ready features to each Apsara and starts development.
 * Based on README.md requirements for AI Governance Control Plane.
 */
...
YOUR TASKS:
1. Read ${PROJECT}/README.md - understand the full project vision
2. components/ui/badge.tsx ALREADY HAS governance status variants (draft, pending_review, approved, rejected). Do NOT add or report "Added governance status variants". Skip to next.
...
Log your work to ${PROJECT}/logs/menaka.log`
  }
};
...
function sendToApsara(agentId, message) {
  try {
    ...
    // Use spawnSync with array args so the message is passed as one argument (no shell escaping).
    // Avoids "Command failed" from broken quoting/newlines and ARG_MAX issues.
    ...
    if (result.error) {
      if (result.error.code === 'ENOENT') {
        return `Error: openclaw CLI not found. Install OpenClaw and add it to PATH, or run from an environment where "openclaw" is available.`;
      }
...
🪷 ═══════════════════════════════════════════════════════════════════════
   SANDARB.AI APSARA DEVELOPMENT ORCHESTRATOR
   Enterprise-Ready AI Governance Control Plane
🪷 ═══════════════════════════════════════════════════════════════════════
`);
...
YOUR TURN: Continuously evaluate what features still need to be built for the mission and problem we're solving. Post a short update (1–2 sentences): what you're doing next or what you suggest. Your first paragraph will be posted to the Team Chat. Then implement or refine. If your scope is complete, say "SCOPE_COMPLETE" in your first line; otherwise keep building.`;
  ...
  ensureLogDir();
  // Only append session start if we didn't already in the last 60 min (avoid repeating system lines on every run)
  const now = Date.now();
  const oneHourMs = 60 * 60 * 1000;
  let shouldLogSession = true;
  if (fs.existsSync(TEAM_CHAT_LOG)) {
    const content = fs.readFileSync(TEAM_CHAT_LOG, 'utf-8');
    const lines = content.trim().split('\n').filter(Boolean);
    const last = lines[lines.length - 1];
    if (last && last.includes('Development session started')) {
      const match = last.match(/^\[([^\]]+)\]/);
      if (match) {
        const lastTs = new Date(match[1]).getTime();
        if (now - lastTs < oneHourMs) shouldLogSession = false;
      }
    }
  }
  if (shouldLogSession) {
