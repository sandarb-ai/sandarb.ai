...
for arg in "$@"; do
  if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
    ...
    echo "               If data/sandarb.sql exists, it is loaded too (generate with ./scripts/generate_sandarb_data.sh)."
...
if [[ -n "$DO_SEED" ]]; then
  [[ -n "$DO_SEED_ONLY" ]] && echo "Running seed only (no build/deploy)..."
  echo "[Seed] Seeding GCP Postgres (schema + sample data)..."
  cd "$REPO_ROOT"
  # Use a Python that has psycopg2 (avoid system pip on macOS/Homebrew externally-managed env)
  SEED_PYTHON=""
  if python3 -c "import psycopg2" 2>/dev/null; then
    SEED_PYTHON="python3"
  elif [[ -x "$REPO_ROOT/.venv/bin/python3" ]] && "$REPO_ROOT/.venv/bin/python3" -c "import psycopg2" 2>/dev/null; then
...
PYSEED
  ...
  if ! "$SEED_PYTHON" scripts/init_postgres.py; then
    [[ -n "$PROXY_PID" ]] && kill "$PROXY_PID" 2>/dev/null || true
    echo "Seed: init_postgres.py failed." >&2
    exit 1
  fi
  if ! "$SEED_PYTHON" scripts/seed_postgres.py; then
    [[ -n "$PROXY_PID" ]] && kill "$PROXY_PID" 2>/dev/null || true
    echo "Seed: seed_postgres.py failed." >&2
    exit 1
  fi
  # Load data/sandarb.sql if present (generated by ./scripts/generate_sandarb_data.sh)
  if [[ -f "$REPO_ROOT/data/sandarb.sql" ]]; then
    echo "  Loading data/sandarb.sql..."
    if ! (cd "$REPO_ROOT" && export DATABASE_URL="$SEED_URL" && "$SEED_PYTHON" -c "
import os, sys
sys.path.insert(0, 'scripts')
from pathlib import Path
from load_db import run_sql_file
run_sql_file(Path('data/sandarb.sql'), os.environ['DATABASE_URL'])
"); then
      [[ -n "$PROXY_PID" ]] && kill "$PROXY_PID" 2>/dev/null || true
      echo "Seed: loading data/sandarb.sql failed." >&2
      exit 1
    fi
  else
    echo "  (No data/sandarb.sql; generate with ./scripts/generate_sandarb_data.sh to load bulk data.)"
  fi
  [[ -n "$PROXY_PID" ]] && kill "$PROXY_PID" 2>/dev/null || true
