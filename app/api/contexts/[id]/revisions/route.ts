...
export async function GET(
  ...
  return withSpan('GET /api/contexts/[id]/revisions', async () => {
    try {
      ...
      logger.error('Failed to fetch revisions', { route: 'GET /api/contexts/[id]/revisions', error: String(error) });
      return NextResponse.json(
...
export async function POST(
  ...
  return withSpan('POST /api/contexts/[id]/revisions', async () => {
    try {
      const { id } = await params;
      ...
      logger.error('Failed to propose revision', { route: 'POST /api/contexts/[id]/revisions', error: String(error) });
      return NextResponse.json(
